...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  % gradient for cross entrohpy loss
  yo = class_prob;
  yh = hid_output;
  wh = model.hid_to_class;
  wi = model.input_to_hid;
  
  % dE/dz_o = y_o - t
  dE_dzo = yo - data.targets;
  % dzo_dwo = yo
  dzo_dwh = yh;
  %dE_dwo = dE_dzo * dzo_dwo'
  dE_dwh = dE_dzo * dzo_dwh';
  
  %dzo_dyh = wh
  dzo_dyh = wh ;
  dE_dyh = dzo_dyh' * dE_dzo;
  
  % logistic derivative
  dyh_dzh = yh .* ( 1- yh);
  
  dE_dzh = dE_dyh .* dyh_dzh;
  
  dzh_dwi = data.inputs;
  dE_dwi = dE_dzh * dzh_dwi' ; 
  
  % weight decay loss
  dE_dwi_1 = wd_coefficient * wi ;
  
  dE_dwh_1 = wd_coefficient * wh;
  
  ret.input_to_hid = dE_dwi_1 + (dE_dwi /  size(hid_input,2));
  ret.hid_to_class = dE_dwh_1 + (dE_dwh /  size(hid_input,2));
end
